    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">
          <card>
            <div slot="header">
              <h4 class="card-title text-primary">Full Template creation</h4>
      ...
      <!-- TODO: Hera customization: this flow is obsolete -->
        ...
        <div class="col-12">
          <md-card>
            <md-steppers :md-active-step.sync="active" md-linear>              
              ...
              <!-- FOURTH STEPPER -->
              <md-step id="fourth" md-label="HTML" :md-error="fourthStepError" :md-done.sync="fourth">                
                <card>
                  <div slot="header">
                ...
                <card v-if="missingSolidityVariableFlag==true">
                  <div v-if="fourthStepError==true && missingSolidityVariableInHTML==true">HINT: Missing HTML </div>
                  <div v-if="missingSolidityVariableFlag==true && missingSolidityVariableInHTML.length>0">HINT: The following solidity variables are not present in the html: {{missingSolidityVariableInHTML}}. You can define a variable in this way: {{solidityVariableExample}} </div>   
                </card>
                <card> 
                  <div slot="header">
    ...
    methods: {
        ...
        if(this.template.text != null && this.template.text != '') { // if editor is not empty
          ...
          for(let i=0; i<this.solidityMustacheVariable.length; i++) { // replace html variable with input type                         
            ...
            if(indices.length < 1) { // if some solidity variable is not written in html
              //console.log('missing '+this.solidityMustacheVariable[i])
          ...
          this.missingSolidityVariableInHTML = false
          this.fourthStepError = null
        } else {
          this.setError('fourth', 'Missing HTML')
        }   
      },
      ...
      generateSemplifiedForm: function(html) {                        
        ...
        for(let i=0; i<htmlVariables.length; i++) {
          ...
          let offset = 0;
          declarationVariable += "<div> HTML variable: "+ htmlVariables[i] + " <input id='" + htmlVariables[i] + "' type='text'  value='{{" + htmlVariables[i] + "}}' oninput=\"updateLabel('" + htmlVariables[i] + "')\">"    
          for(let j=0; j<htmlVariableIndexes.length; j++) {
            let variable = "<output name='" + htmlVariables[i] +"'></output> "   
            let html_temp = html   
            html = this.replaceVariables(html_temp, htmlVariableIndexes[i]+offset, "[["+htmlVariables[i]+"]]", variable)
