    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">
          <card>
            <div slot="header">
              <h4 class="card-title text-primary">Modify HTML template <b>{{template.templatename}}</b></h4>
          ...
          <card>
            <md-steppers :md-active-step.sync="active" md-linear>
              ...
              <md-step id="fourth" md-label="HTML" :md-error="fourthStepError" :md-done.sync="fourth">                
                ...
                <card v-if="fourthStepError==true">
                  <div v-if="fourthStepError==true">HINT: Missing HTML </div>
                  <div v-if="missingSolidityVariableFlag==true">HINT: The following solidity variables are not present in the html: {{missingSolidityVariableInHTML}}. You can define a variables in this way: {{solidityVariableExample}} </div>   
                </card>  
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      checkFourth: function() { //controlla se tutte le variabili solidity sono nell'html
        
        let error = false;
        this.html = this.template.text
        ...
        if(this.html != null && this.html != '') {
          this.missingSolidityVariableInHTML = []
          for(let i=0; i<this.solidityMustacheVariable.length; i++) { // replace html variable with input type       
            let indices = this.getIndicesOf("{{" + this.solidityMustacheVariable[i] + "}}", this.html)
            if(indices.length < 1) {
          ...
          this.completeFormHTML = this.generateCompleteForm(this.template.text)          
          this.template.html = this.completeFormHTML
          this.setDone('fourth', 'sixth')
      ...
      generateSemplifiedForm: function(html) {        
        ...
        for(let i=0; i<htmlVariables.length; i++) {
          ...
          let offset = 0;
          declarationVariable += "<div>"+ htmlVariables[i] + ": <input id='" + htmlVariables[i] + "' type='text'  value='{{" + htmlVariables[i] + "}}' oninput=\"updateLabel('" + htmlVariables[i] + "')\">"              
